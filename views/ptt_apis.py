...
#        data = ptt_articles.query.filter (ptt_articles.board_name == board_name).all ()
    ...
    for i , item in enumerate (data):
        row.append ( dict ( Id = i+1 , push_num = item.push_count , url = item.url , title = item.title , date = item.article_date ) )
...
def select_ptt_data():
    ...
    if request.method == 'POST':
        ...
        if count == 30 and (now - one.update_time).total_seconds() < 14400:
            ...
            return redirect (url_for ('ptt' , board_name = board_name , push_num = push_num , search_title = search_title))
    else:
